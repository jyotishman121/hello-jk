...
<body class="<?php echo trim($classesReturn); ?>">
	...
	<script>
		...
		window.ypOption = {
				...
				"fixed_left_bar"				: <?php echo yp_get_live_option("fixed_left_bar"); ?>, // Fixed Left Bar
				...
				iframeNode.addEventListener("load", function() {
					...
					if(window.bMode !== true){
						if(iframeReady || iframeURL.indexOf("yellow_pencil_frame") == -1){
							...
							// show loading
							document.querySelector(".yp-iframe-loader").style.display = "block";
						  ...
						  parentURL = parentURL.toString().split("href=")[0] + "href=";
							...
							xhr.onreadystatechange = function () {
							    ...
							    if (this.status == 200) {
											...
											data = data.querySelector('#yp_page_details').innerHTML;
					...
					if(editorData !== null){
						...
						iframeBody.insertAdjacentHTML('beforeend', '<div id="yp-animate-data">'+iframeHead.querySelector("#yp-animate-data").innerHTML+'</div>');
						iframeHead.removeChild(iframeHead.querySelector("#yp-animate-data"));
					}

					// CSS Loader
					function yp_load_style(link, i, length){
						...
						style.onload = function(){
							yp_load_note("Loading Styles", 39 + parseInt(21*i/(length - 1)));
						};
					...
					iframeHead.insertAdjacentHTML('beforeend', "<link rel='stylesheet' id='yellow-pencil-frame'  href='<?php echo plugins_url('css/frame.css?ver='.YP_VERSION.'', __FILE__); ?>' type='text/css' media='all' />");
					iframeHead.insertAdjacentHTML('beforeend', "<link rel='stylesheet' id='yellow-pencil-animate'  href='<?php echo plugins_url('../public/css/animate.css?ver='.YP_VERSION.'', __FILE__); ?>' type='text/css' media='all' />");

					// Loading.
					for(var i = 0; i < styles.length; i++){
						yp_load_style(styles[i], i, styles.length);
					}

					var scripts = [
						"<?php echo plugins_url( 'js/ace/editor.js?ver='.YP_VERSION.'' , __FILE__ ); ?>",
						...
						"<?php echo plugins_url( 'js/addons.js?ver='.YP_VERSION.'' , __FILE__ ); ?>",
					...
					function yp_start_editor(){
						...
						setTimeout(function(){
							...
							document.querySelector(".yp-iframe-loader").style.display = 'none';
						}, 350);

					}

					// JS loader
					function yp_load_script(src, i, length) {
						...
						script.onload = function(){
							yp_load_note("Loading Scripts", 60+parseInt(38*i/(length - 1)));
							if(i === (length - 1)){
								yp_start_editor();
							}
						};
					...
					for(var i = 0; i < scripts.length; i++){
						yp_load_script(scripts[i], i, scripts.length);
					}
