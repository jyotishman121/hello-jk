(function ($) {
  'use strict';
  $.widget("cp.UseyourDriveDashboard", {
    options: {
      $container_totals: $('#useyourdrive-totals'),
      $container_events_chart: $('#useyourdrive-events-chart'),
    ...
    _initEvents: function () {
      ...
      $('body').on('click', '#useyourdrive-modal-action a.open-entry-details', function (e) {
        var id = $(this).data('entry-id');
        var account_id = $(this).data('account-id');
        self.viewEntryDetails(id, account_id);
      });
      ...
      $('body').on('click', '#useyourdrive-modal-action a.open-user-details', function (e) {
        var id = $(this).data('user-id');
        self.viewUserDetails(id);
      });
    ...
    clearStatistics: function () {
      ...
      $.ajax({
        ...
        data: {
          action: 'useyourdrive-reset-statistics',
    ...
    loadTotalsData: function ($container_totals, detail, id) {
      ...
      $container_totals.find('.useyourdrive-counter span').css({'background-color': '', 'color': ''});
    ...
    _setTotalsData: function ($container_totals, dataset) {
      ...
      $.each($container_totals.find('.useyourdrive-counter'), function () {
    ...
    loadFullLog: function () {
      ...
      self.fullLog = self.options.$container_full_log.DataTable({
        ...
        "ajax": {
          "url": self.options.ajax_url,
          data: function (d) {
            d.action = 'useyourdrive-event-stats';
            d.type = 'full-log';
    ...
    loadDetailedLog: function ($table, detail, id) {
      ...
      $table.DataTable({
        ...
        "processing": true,
        "serverSide": true,
        "ajax": self.options.ajax_url + '?action=useyourdrive-event-stats&type=full-log&detail=' + detail + '&id=' + id + '&_ajax_nonce=' + self.options.admin_nonce
      });
    },
    ...
    openModal: function (detail, id, account_id) {
      ...
      var modalbuttons = '';
      modalbuttons += '<button class="simple-button blue useyourdrive-modal-cancel-btn secondary" data-action="cancel" type="button" onclick="modal_action.close();" title="' + self.options.str_close_title + '" >' + self.options.str_close_title + '</button>';
      ...
      var modalfooter = $('<div class="useyourdrive-modal-footer" style="display:none"><div class="useyourdrive-modal-buttons">' + modalbuttons + '</div></div>');
      var modaldialog = $('<div id="useyourdrive-modal-action" class="UseyourDrive useyourdrive-modal useyourdrive-modal80 ' + self.options.content_skin + '"><div class="modal-dialog"><div class="modal-content"><div class="loading"><div class="loader-beat"></div></div></div></div></div>');
      ...
      /* Load Log*/
      self.loadDetailedLog($('#useyourdrive-modal-action .modal-content table'), detail, id);
...
(function ($) {
  $(".UseyourDriveDashboard").UseyourDriveDashboard(UseyourDrive_Dashboard_vars);
})(jQuery);