...
class UYDGoogle_Config
{
    ...
    /**
     * Create a new UYDGoogle_Config. Can accept an ini file location with the
    ...
    public function __construct($ini_file_location = null)
    {
        $this->configuration = [
            ...
            'application_name' => 'WPCP '.USEYOURDRIVE_VERSION,

            // Which Authentication, Storage and HTTP IO classes to use.
            'auth_class' => 'UYDGoogle_Auth_OAuth2',
            'io_class' => self::USE_AUTO_IO_SELECTION,
            'cache_class' => 'UYDGoogle_Cache_File',
            'logger_class' => 'UYDGoogle_Logger_Null',
            ...
            'classes' => [
                ...
                'UYDGoogle_Logger_Abstract' => [
                    'level' => 'debug',
                    'log_format' => "[%datetime%] %level%: %message% %context%\n",
                    'date_format' => 'd/M/Y:H:i:s O',
                    'allow_newlines' => true,
                ],
                'UYDGoogle_Logger_File' => [
                    'file' => 'php://stdout',
                    'mode' => 0640,
                    'lock' => false,
                ],
                'UYDGoogle_Http_Request' => [
                ...
                'UYDGoogle_Service_Exception' => [
                    'retry_map' => [
                        ...
                        '403' => self::TASK_RETRY_ALWAYS,
                        'rateLimitExceeded' => self::TASK_RETRY_ALWAYS,
                        'userRateLimitExceeded' => self::TASK_RETRY_ALWAYS,
                    ],
                ],
                'UYDGoogle_IO_Exception' => [
                    'retry_map' => !extension_loaded('curl') ? [] : [
                        CURLE_COULDNT_RESOLVE_HOST => self::TASK_RETRY_ONCE,
                        CURLE_COULDNT_CONNECT => self::TASK_RETRY_ONCE,
                        CURLE_OPERATION_TIMEOUTED => self::TASK_RETRY_ALWAYS,
                        CURLE_SSL_CONNECT_ERROR => self::TASK_RETRY_ONCE,
                        CURLE_GOT_NOTHING => self::TASK_RETRY_ALWAYS,
                    ],
                ],
                // Set a default directory for the file cache.
                'UYDGoogle_Cache_File' => [
                    'directory' => sys_get_temp_dir().'/UYDGoogle_Client',
                ],
            ],
        ];
        if ($ini_file_location) {
            ...
            if (is_array($ini) && count($ini)) {
                ...
                if (isset($ini['classes'], $this->configuration['classes'])) {
     ...
     * $config->setClassConfig('UYDGoogle_Cache_File',
     ...
     * the hd values within an id token (@see UYDGoogle_Auth_LoginTicket) after sign
